...
public class MainMenu {
    ...
    private JButton createStyledButton(String text) {
        JButton button = new JButton(text) {
            ...
            private final Timer fadeTimer = new Timer(16, e -> {
                glowAlpha = targetState
                        ? Math.min(1f, glowAlpha + 0.08f)
                        : Math.max(0f, glowAlpha - 0.08f);
                repaint();
...

            {
                addMouseListener(new MouseAdapter() {
                    @Override public void mouseEntered(MouseEvent e) {
                        targetState = true; fadeTimer.start();
                        setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
                    }
                    @Override public void mouseExited(MouseEvent e) {
                        targetState = false; fadeTimer.start();
                        setCursor(Cursor.getDefaultCursor());
                    }
                    @Override public void mousePressed(MouseEvent e)  { glowAlpha = 1f; repaint(); }
                    @Override public void mouseReleased(MouseEvent e) { targetState = false; fadeTimer.start(); }
                });
            }
            ...
            protected void paintBorder(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                g2.setColor(interpolateColor(new Color(120, 72, 48), new Color(220, 170, 80), glowAlpha));
